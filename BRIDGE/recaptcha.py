...
def find_chrome_executable() -> Optional[str]:
    ...
    # Playwright-managed Chromium (common on headless Linux servers)
...
async def get_recaptcha_v3_token() -> Optional[str]:
    ...
    try:
        ...
        # Config-driven headless mode; Xvfb users can set false for Turnstile pass-through
        try:
            ch = config.get("camoufox_fetch_headless", None)
            user_headless = bool(ch) if ch is not None else None
        except Exception:
            user_headless = None
        want_headless = user_headless if user_headless is not None else True
        async with _m().AsyncCamoufox(headless=want_headless, main_world_eval=True) as browser:
