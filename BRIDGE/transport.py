...
class BrowserFetchStreamResponse:
    ...
    async def aread(self) -> bytes:
        if self._lines_queue is not None:
            # Buffer any already-running browser stream for callers that use the
            # response body API (non-streaming adapters and tests).
            collected = []
            ...
            for attempt in range(max_recaptcha_attempts):
                ...
                if fetch_task.done() and meta is None:
                    ...
                    try:
                        ...
                        # Prefer a completed buffered fetch result over queue metadata.
                        res = None
                        try:
                            res = fetch_task.result()
                        except Exception as exc:
                            res = {"status": 502, "text": f"FETCH_EXCEPTION: {exc}"}
...
async def fetch_lmarena_stream_via_camoufox(
    ...
    provisional_user_id = str(config.get("provisional_user_id") or cookie_map.get("provisional_user_id") or "").strip()
    grecaptcha_cookie = str(cookie_map.get("_GRECAPTCHA") or "").strip()
    # Prefer the current browser session over stale auth_tokens entries.
    effective_auth_token = str(cookie_map.get("arena-auth-prod-v1") or auth_token or "").strip()
    ...
    if not effective_auth_token:
        effective_auth_token = str(_m().EPHEMERAL_ARENA_AUTH_TOKEN or "").strip()
    if effective_auth_token:
        desired_cookies.extend(_arena_auth_cookie_specs(effective_auth_token))
    user_agent = _m().normalize_user_agent_value(config.get("user_agent"))
    ...
    while True:
        try:
            ...
            if needs_launch:
                ...
                current_auth_cookie = str(cookie_map.get("arena-auth-prod-v1") or "").strip()
                ...
                for value, name in cookie_definitions:
                    if value:
                        for _domain in (".lmarena.ai", ".arena.ai"):
                            desired_cookies.append({"name": name, "value": value, "domain": _domain})
                # provisional_user_id is host-scoped in some Arena responses;
                # use the validated url/path specs instead of a domain-only cookie.
                if provisional_user_id:
                    desired_cookies.extend(_provisional_user_id_cookie_specs(provisional_user_id))
                # Seed auth first, even when the other cookie list is empty.
                if current_auth_cookie:
                    desired_cookies.extend(_arena_auth_cookie_specs(current_auth_cookie))
                if desired_cookies:
                    try:
                        existing_names: set[str] = set()
                        try:
                ...
                try:
                    ...
                    # A combined cookie in browser_cookies is authoritative even
                    # when the stale persistent context contains an older cookie.
                    # This is the path used by WEB /api/tokens and avoids silently
                    # selecting an expired round-robin entry.
                    configured_auth = str((cfg.get("browser_cookies") or {}).get("arena-auth-prod-v1") or "").strip()
                    if configured_auth and not has_fresh_existing:
                        await context.add_cookies(_arena_auth_cookie_specs(configured_auth))
                        existing_auth = configured_auth
                        has_fresh_existing = True
