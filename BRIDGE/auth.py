...
async def refresh_arena_auth_token_via_supabase(old_token: str, *, anon_key: Optional[str] = None) -> Optional[str]:
    ...
    # Fallback: read persisted anon key from config (survives restarts)
    if not key:
        try:
            cfg = _m().get_config()
            key = str((cfg or {}).get("supabase_anon_key") or "").strip()
        except Exception:
            pass
    if not key:
        return None
...
def get_next_auth_token(exclude_tokens: set = None, *, allow_ephemeral_fallback: bool = True):
    ...
    # The WEB dashboard stores the freshest browser session separately from
    # the historical round-robin pool. Prefer it when it is currently valid.
    browser_cookie = ""
    if isinstance(config.get("browser_cookies"), dict):
        browser_cookie = str(config["browser_cookies"].get("arena-auth-prod-v1") or "").strip()
    if browser_cookie and not is_arena_auth_token_expired(browser_cookie, skew_seconds=0):
        if not exclude_tokens or browser_cookie not in exclude_tokens:
            if bool(config.get("persist_arena_auth_cookie")):
                try:
                    config["auth_tokens"] = [browser_cookie]
    ...
    if not auth_tokens:
        ...
        if isinstance(cookie_store, dict):
            ...
            if token and not is_arena_auth_token_expired(token):
                auth_tokens = [token]
                # Persist the fallback token when the deployment explicitly opts in.
                # This keeps browser-cookie recovery durable without mutating config by default.
                if bool(config.get("persist_arena_auth_cookie")):
                    try:
                        config["auth_tokens"] = [token]
                        _m().save_config(config, preserve_auth_tokens=False)
                    except Exception as exc:
                        _m().debug_print(f"⚠️ Could not persist browser auth cookie: {exc}")
