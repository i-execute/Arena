...
class TestChromeFetchRobustness(unittest.IsolatedAsyncioTestCase):
    async def test_fetch_via_chrome_retries_cloudflare(self):
        ...
        with patch("playwright.async_api.async_playwright", return_value=mock_playwright), \
             patch("BRIDGE.main.find_chrome_executable", return_value="/path/to/chrome"), \
             patch("BRIDGE.main.get_config", return_value={}), \
             patch("BRIDGE.main.get_recaptcha_settings", return_value=("key", "action")), \
             patch("BRIDGE.main.click_turnstile", AsyncMock(return_value=True)) as mock_click, \
             patch("BRIDGE.main.asyncio.sleep", AsyncMock()):
    ...
    async def test_fetch_via_chrome_sends_payload_recaptcha_token_in_headers(self):
        ...
        with patch("playwright.async_api.async_playwright", return_value=mock_playwright), patch(
            "BRIDGE.main.find_chrome_executable", return_value="/path/to/chrome"
        ), patch("BRIDGE.main.get_config", return_value={}), patch(
            "BRIDGE.main.get_recaptcha_settings", return_value=("key", "action")
        ), patch(
            "BRIDGE.main.click_turnstile", AsyncMock(return_value=True)
        ), patch(
            "BRIDGE.main.asyncio.sleep", AsyncMock()
        ):
            resp = await main.fetch_lmarena_stream_via_chrome(
                "POST",
                "https://arena.ai/api",
